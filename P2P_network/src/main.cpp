...
void* send_chunk(void* void_arg) {
    ...
    connect_to_client: {
        ...
        int ret = perror_check(
                poll(&poll_fd, 1, arg.timeout_milli));
        ...
        if (client_socket < 0) {
            ...
            cout << my_get_time() << "Client " << client_ip_string << " connected" << endl;
        }
    }
    ...
    while (true) {
        ...
        count_of_bytes_to_send = std::min(data->bytes_to_send_in_timeframe, remaining_file_size);
        ...
        if (data->responses_to_retransmit.size()) {
            ...
            if (diff_milli > 1000) {
                ...
                if (to_send_packet.header.ttl >= 0) {
                    ...
                    for (Node& neighbor : data->neighbors) {
                        if (neighbor.id == get<2>(response_tup)) {
                            continue;
                        }
            ...
            for (int i = 0; i < chunks.size(); i++) {
...

                {
                    ...
                    Send_Chunk_Args* args = new Send_Chunk_Args({
                        data,
                        received_packet.file + ".ch" + to_string(chunk),
                        server_socket,
                        timeout_milli,
                        chunk
                    });
            ...
            while (difftime(time(0), request_start) < request_timeout) {
                ...
                if (!seen_chunks[temp_packet.chunk]) {
                    ...
                    Receive_Chunk_Args* args = new Receive_Chunk_Args{
                        ...
                        temp_packet.port,
                        data->file.metadata_file_name + ".ch" + to_string(temp_packet.chunk)
                    };
...
int main(int argc, char** argv) {
    ...
    init: {
        ...
        pthread_create(&data.receive_udp_packets_thread, 0, receive_udp_packets, &data);
        pthread_create(&data.retransmit_udp_packets_thread, 0, retransmit_udp_packets, &data);
        ...
        pthread_create(&data.manage_bytes_to_send_thread, 0, manage_bytes_to_send, &data);
    }
